Shader "CustomEffects/Blur"
{
    HLSLINCLUDE
        ...
        float4 Blur (Varyings input) : SV_Target
        {
            ...
            float3 color = 0;
            float blurPixelsY = _VerticalBlur * _ScreenParams.y;
            float blurPixelsX = _VerticalBlur * _ScreenParams.x;
            
            for(float i = -BLUR_SAMPLES_RANGE; i <= BLUR_SAMPLES_RANGE; i++)
            {
                float2 sampleOffset = float2 (0, (blurPixelsY / _BlitTexture_TexelSize.w) * (i / BLUR_SAMPLES_RANGE));
